...
class PlaylistListView(View):
    ...
    def create_embed(self) -> discord.Embed:
        embed = discord.Embed(
            title=f"Playlists Saved: {len(self.playlists)}",
...
class PlaylistTrackView(View):
    def __init__(self, tracks: List[Any], playlist_name: str):
        super().__init__(timeout=180)
        self.tracks = list(tracks)
        self.playlist_name = playlist_name

class PlaylistMemberView(View):
    def __init__(self, tracks: List[Any], playlist_name: str):
        super().__init__(timeout=180)
        self.tracks = list(tracks)
        self.playlist_name = playlist_name
        ...
        self.total_pages = math.ceil(len(self.tracks) / self.tracks_per_page)
